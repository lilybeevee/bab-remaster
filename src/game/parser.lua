...
function parser.parse(words, dir, no_verb_cond)
  local extra_words = {}
  for i = #words,1,-1 do
  ...
  while words[1] and words[1].type and words[1].type.verb do
    local verb, words_ = parser.findVerbPhrase(table.copy(words), extra_words, dir, true, false, no_verb_cond)
...
function parser.findUnit(words, extra_words_, dir, outer, no_verb_cond, is_subject)
  local extra_words = {}
  ...
  while words[1].type and (words[1].type.cond_prefix or (words[1].type.cond_compare and not is_subject)) do
    local prefix = table.copy(words[1])
    table.remove(words, 1)
    if #words == 0 then return end
  ...
  local words_
  unit, words_ = parser.findClass(table.copy(words))
  ...
  while words[1] and words[1].type and (words[1].type.cond_infix or (words[1].type.direction and words[2] and words[2].name == "arond")) and (first_infix or enclosed) and (not no_verb_cond or not words[1].type.verb) do
    local infix = table.copy(words[1])
    ...
    if infix.type.cond_infix_verb then
      ...
      if infix.type.cond_infix_dir and words[1].type.direction then
        ...
        local other, words_ = parser.findUnit(table.copy(words), extra_words, dir)
        if not other then
        ...
        while enclosed and words[1] and words[1].type["and"] and words[2] and words[3] and (words[2].type.object or words[2].type.parenthesis or (words[3].name == "text" or words[3].name == "textn't")) do
          ...
          table.remove(words, 1)
          if #words == 0 then break end
          local other, words_ = parser.findUnit(table.copy(words), extra_words)
          if not other then
            if parenthesis then
              return
            end
            unit.conds = conds
            table.merge(extra_words_, extra_words)
            found = {unit, words}
            return unit, words
          end
          words = words_
...
function parser.findClass(words)
  local prefix
...
function parser.findVerbPhrase(words, extra_words_, dir, enclosed, noconds, no_verb_cond)
  local extra_words = {}
  local objects = {}
  local verb = table.copy(words[1])
  ...
  while true do
    local valid
    if (verb.type.verb_class or (verb.type.verb_unit and noconds)) and parser.findClass(table.copy(words)) then
    ...
    elseif verb.type.verb_property and words[1].type.property then
      ...
      valid = true
    elseif verb.type.verb_unit and not noconds and parser.findUnit(table.copy(words), extra_words, dir, enclosed, no_verb_cond) then
      table.insert(objects, found[1])
      words = found[2]
      valid = true
...
function parser.findLetterSentences(str, index_, sentences_, curr_sentence_, start_) --copied from parser_old.lua
  ...
  local curr_sentence = table.copy(curr_sentence_ or {})
  local start = start_ or false
  --print("start of parser.findLetterSentences:",str,index,fullDump(sentences),fullDump(curr_sentence),start, sentences.start, sentences.endd, sentences.both, sentences.middle)
  ...
  if #curr_sentence == 0 and not index == string.len(str) then --go to the next letter if we don't have anything in this one... or if we do
    parser.findLetterSentences(str, index+1, sentences, {}, false)
  end
  ...
  for i=0,string.len(str)-index do
    ...
    if text_in_tiles[substr] then
      ...
      if index+i == string.len(str) then --last letter, this sentence is valid to connect to other words
        ...
        if start then
          table.insert(sentences.both, table.copy(curr_sentence)) --connected to both the start and end, so the parser has to treat this like a string of words
        else
          table.insert(sentences.endd, table.copy(curr_sentence))
        end
        ...
        if start then
          table.insert(sentences.start,table.copy(curr_sentence))
        else
          table.insert(sentences.middle,table.copy(curr_sentence))
        end
        parser.findLetterSentences(str, index+i+1, sentences, curr_sentence, start) --we got one word, now keep going
    ...
    --[[if (index < string.len(str)) then
      parser.findLetterSentences(str, index+1, sentences, curr_sentence, start)
    end]]
  end
